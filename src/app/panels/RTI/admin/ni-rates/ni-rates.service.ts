...
import {
  NiRatesResponse,
  SaveNiRate,
} from 'src/app/models/admin/niRates.model';
...
export class NiRatesService {
  ...
  getNiRates(): Observable<NiRatesResponse> {
    return this.http.get<NiRatesResponse>(`${this.apiUrl}/Admin/NiRates/`);
  }

  saveNiRate(niRate: SaveNiRate): Observable<SaveNiRate> {
    const cleanedPayload = cleanPayload(niRate);
    if (cleanedPayload.id) {
      return this.http.put<SaveNiRate>(
        `${this.apiUrl}/Admin/NiRate/${cleanedPayload.id}`,
        cleanedPayload
      );
    } else {
      return this.http.post<SaveNiRate>(
        `${this.apiUrl}/Admin/NiRate/`,
        cleanedPayload
      );
    }
  }
